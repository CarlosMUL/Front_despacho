...
Estuve más a cargo del repositorio `Front_despacho`, principalmente revisando la configuración de secretos y variables de entorno de ese servicio. Nos coordinábamos como equipo a través de reuniones presenciales, y cuando no era posible juntarnos, seguíamos coordinándonos por Discord.
...
Mi participación estuvo concentrada en revisar el frontend y su conexión con los backends, principalmente dentro del repositorio `Front_despacho`. Revisé cómo el frontend (React + Vite) consume los endpoints REST expuestos por los servicios de Ventas y Despachos, ayudé a verificar que las llamadas HTTP configuradas por variables de entorno (`DESPACHOS_BACKEND_HOST`, `VENTAS_BACKEND_HOST`) apuntaran correctamente a cada servicio, y revisé la configuración de secretos y variables de entorno de ese repositorio. También participé de una demo del equipo donde se mostró en vivo el flujo completo: cómo el frontend llama a los endpoints y qué datos devuelve cada uno.

Además, configuré algunas Task Definitions y servicios en la consola de Amazon ECS durante el proceso de despliegue.
...
Mi aporte estuvo en la revisión funcional del lado frontend-backend: confirmar que las conexiones entre la interfaz y los servicios REST funcionaran como se esperaba, y apoyar en la configuración de tasks y servicios en ECS durante los despliegues, más que en el diseño completo de la infraestructura o el pipeline.

## 7. Qué aspectos técnicos domino y puedo defender

Puedo explicar con confianza la comunicación entre el frontend, los backends y la base de datos: cómo el frontend (React + Vite + Nginx) consume los endpoints REST de Ventas (`/api/v1/ventas`) y Despachos (`/api/v1/despachos`), cómo esas llamadas se configuran mediante variables de entorno, y cómo ambos backends terminan escribiendo/leyendo desde la misma instancia de Amazon RDS MySQL, cada uno en su base de datos lógica separada (`ventas_db`, `despachos_db`).

También puedo explicar a nivel general qué es una Task Definition en ECS y cómo se asocia a un servicio dentro del clúster, ya que participé en configurar algunas durante los despliegues.

Para las partes de pipeline CI/CD y seguridad de red, mi comprensión es más general (a nivel de qué hace cada pieza y por qué se eligió), no de haberlas configurado yo mismo.
